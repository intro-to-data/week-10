...
- **Question:** Is there a linear relationship between the weight of a diamond and the price of the diamond?
...
- **Question:** While not perfect, taking the log of both carat and price gives
  us a much more linear relationship. In the code sample above, I take the log of both carat and price. Was this necessary (yes/no) and why (use ggplots to support your answer).
- **Answer:** 
- Helpful Hint:
...
diamonds |>
  ggplot(aes(x = carat_log, y = price)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = lm)
...
- **Question:** Create a model, `lm_simple` which models `price_log` as a function of `carat_log`.
...
lm_simple <-
  linear_reg() |>
  fit(price_log ~ carat_log, data = diamonds)
...
lm_simple |>
  ...
  select(carat, carat_log, price, price_log, .pred, pred) |>
  ggplot(aes(x = pred)) +
  geom_density()
