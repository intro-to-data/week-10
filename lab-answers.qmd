...
- **Answer:** The answer is to draw a scatter plot of weight as a function of wing lenght. This means weight should be on the Y Axis and Wing Length is on the X Axis. And they should observe in their answer that the relationship between the X and Y variable does appear to be linear.
...
lm_wing <- linear_reg() |> fit(weight ~ wing, data = hawks)
tidy(lm_wing)
glance(lm_wing)
```
...
lm_tail <- linear_reg() |> fit(weight ~ tail, data = hawks)
tidy(lm_tail)
glance(lm_tail)
```
...
- **Answer:** They aren't identical, but they are similar.
...
- **Question:** Plot the distribution of the residuals from the lm_wing model using geom_density and geom_qq. Do these residual values appear to be normally distributed?
- **Answer:** The residuals seem pretty normal based on the density plot, but the QQ plot makes it  cleare the highest and lowest values are leveraging the model.
...
hawks_lm_wing <-
  lm_wing |>
  augment(new_data = hawks)

ggplot(hawks_lm_wing, aes(.resid)) +
  geom_density()

ggplot(hawks_lm_wing, aes(sample = .resid)) +
  geom_qq() +
  geom_qq_line()
```
...
- **Answer:** I am looking for .resid on the Y Axis and .pred on the X Axis, but if they get it backwards, don't take off more than a point or two. The answer is NO, it is not homogenous and we still see the same three groups we saw earlier.
...
```{r}
lm_wing_species <- linear_reg() |> fit(weight ~ wing + species, data = hawks)
tidy(lm_wing_species)
glance(lm_wing_species)
```
...
- **Answer:** Yes, but it is still weird.
...
```{r}
lm_wing_species_interactions <- linear_reg() |> fit(weight ~ wing * species, data = hawks)
tidy(lm_wing_species_interactions)
glance(lm_wing_species_interactions)
```
